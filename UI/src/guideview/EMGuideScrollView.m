...
@implementation EMGuideScrollView
...
- (id)initWithItems:(NSArray *)items;
{
    ...
    if (self) {
        ...
        {
            NSObject<EMGuideScrollModel> *item = [items objectAtIndex:i];
            NSAssert([item conformsToProtocol:@protocol(EMGuideScrollModel)], @"item 没有实现 EMGuideScrollModel");
            
            UIView<EMGuideScrollUpdating> *cell = [EMGuideScrollView guidCellForGuidScrollView:self atIndex:i withObject:item];
...
- (void)addTapGestureRecognizerOnLastCell
{
    if ([_cells count] > 0)
    {
        UIView<EMGuideScrollUpdating> *cell = [_cells lastObject];
...
+ (id)showGuideScrollViewWithItems:(NSArray *)items
                  ...
                  autoDismissDelay:(CGFloat)delay
                        completion:(void (^)(void))completion
{
    EMGuideScrollView *guidView = [[EMGuideScrollView alloc] initWithItems:items];
...
+ (UIView<EMGuideScrollUpdating> *)guidCellForGuidScrollView:(EMGuideScrollView *)guidView
                                                     atIndex:(int)index
                                                  withObject:(NSObject<EMGuideScrollModel> *)object
{
    if ([object isKindOfClass:[UIImage class]])
    {
        EMGuideScrollImageCell *cell = [[EMGuideScrollImageCell alloc] init];
        [cell updateGuideViewWithModel:object];
        return cell;
    }
    else
    {
        UIView<EMGuideScrollUpdating> *cell = [[[object cellClass] alloc] init];
        [cell updateGuideViewWithModel:object];
        return cell;
    }
}
