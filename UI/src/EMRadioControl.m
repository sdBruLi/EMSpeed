        ...
        radioTitles:(NSArray *)titles
{
    ...
    if (self) {
        ...
        {
            EMRadio *radioBtn = [[self radioClass] radioWithTitle:[_titles objectAtIndex:i] target:self action:@selector(radioBtnClick:)];
...
-(void)radioBtnClick:(id)sender
{
    ...
    {//将之前选中的不选中
        ...
        EMRadio *preButton     = (EMRadio *)[self viewWithTag:kRadioButtonTag + _selectedIndex];
        EMRadio *currentButton = (EMRadio *)[self viewWithTag:kRadioButtonTag + index];
...
- (void)setOnImage:(UIImage *)onImage
{
    if (onImage) {
        ...
        for (int i=0; i<[_radios count]; i++) {
            EMRadio *r = [_radios objectAtIndex:i];
            r.onImage = onImage;
...
@implementation EMRadio

+ (EMRadio *)radioWithTitle:(NSString *)title
                    onImage:(UIImage *)onImage
                   offImage:(UIImage *)offImage
                     target:(id)target
                     action:(SEL)selector
{
    EMRadio *radioBtn = [EMRadio buttonWithType:UIButtonTypeCustom];
    ...
    radioBtn.onImage = onImage ? onImage : [EMRadio defaultOnImage];
    radioBtn.offImage = offImage ? offImage : [EMRadio defaultOffImage];
...
+ (EMRadio *)radioWithTitle:(NSString *)title
                     ...
                     action:(SEL)selector
{
    return [EMRadio radioWithTitle:title onImage:nil offImage:nil target:target action:selector];
}

+ (UIImage *)defaultOnImage
{
    return [UIImage imageNamed:EMUIResName(@"radio_on")];
}

+ (UIImage *)defaultOffImage
{
    return [UIImage imageNamed:EMUIResName(@"radio_off")];
}
