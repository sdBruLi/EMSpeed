...
#import "EMSystemAlertPopupView.h"
#import "EMPopupViewManager.h"
#import "EMPopupView.h"

@interface EMSystemAlertPopupView() {
    BOOL _isShowed;
}
...
@implementation EMSystemAlertPopupView
...
- (void)show
{
    ...
    if (self.managedByQueue) {
        // 然后显示队列中第一个
        EMPopupViewManager *sharedManager = [EMPopupViewManager sharedManager];
...
- (void)dequeueAndShowNextPopupView
{
    if (self.managedByQueue) {
        ...
        EMSystemAlertPopupView *popupView = (EMSystemAlertPopupView *)[sharedManager firstPopupView];
