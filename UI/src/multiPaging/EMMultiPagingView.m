...
@implementation EMMultiPagingView
...
- (void)loadViews
{
    self.pageControlAlignment = EMMultiPagingPageControlAlignmentCenter;
    
    self.clipsToBounds = YES;
    _placeholderImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:EMUIResName(@"overlay")]];
...
- (void)layoutSubviews
{
    ...
    switch (self.pageControlAlignment) {
        case EMMultiPagingPageControlAlignmentLeft:
            _pageControl.center = CGPointMake(45, rect.size.height - 12);
            break;
        case EMMultiPagingPageControlAlignmentCenter:
            _pageControl.center = CGPointMake(CGRectGetMidX(rect), rect.size.height - 12);
            break;
        case EMMultiPagingPageControlAlignmentRight:
...
- (void)setPageItems:(NSArray *)pageItems
{
    ...
    {
        NSObject<EMPageModel> *aData = [_pageItems objectAtIndex:i];
        UIView<EMPageUpdating> *page = [[EMPageImageView alloc] initWithFrame:rect];
...
- (void)didTapScrollView:(UITapGestureRecognizer *)recognizer
{
    if (_delegate)
    {
        ...
        if ([_delegate respondsToSelector:@selector(adView:didSelectAdData:)])
        {
            ...
            NSObject<EMPageModel> *aData = [_pageItems objectAtIndex:index];
